...
class Controller():
    ...
    def loadSettings(self):
        ...
        self.originalSettings = Settings(self.settingsFile)             # save the original state so that we can know if something has changed when we exit NinjaArt      
...
#        self.view.settingsWidget.setSettings(Settings(self.settingsFile))
    ...
    def applySettings(self, newSettings):                               # call this function when clicking 'apply' in the settings menu (after validation)
        ...
        self.view.applyAppearance(newSettings)
...
#       menu.addAction("Run custom command")
    ...
    def handlePortAction(self, targets, actions, terminalActions, action, restoring):
        ...
        if action.text() == 'Send to NXC':
            for ip in targets:
                self.view.createNewNxcTab(ip[0], ip[1], ip[3])          # ip[0] is the IP, ip[1] is the port number and ip[3] is the service name
            return
...
#           print('> queue is empty')
    ...
    def processFinished(self, qProcess):
        ...
        try:
            ...
            if 'nxc' in qProcess.name:                                  # find the corresponding nxc widget and tell it to update its UI
                self.view.findFinishedNxcTab(str(self.logic.getPidForProcess(str(qProcess.id))))
