...
class Logic():
    def __init__(self):    
        ...
        self.createTemporaryFiles()                                     # creates temporary files/folders used by NinjaArt
    ...
    def createTemporaryFiles(self):
        try:
            ...
            tf = tempfile.NamedTemporaryFile(suffix=".sprt",prefix="ninjaart-", delete=False)         # to store the database file
            self.outputfolder = tempfile.mkdtemp(suffix="-tool-output",prefix="ninjaart-")            # to store tool output of finished processes
            self.runningfolder = tempfile.mkdtemp(suffix="-running",prefix="ninjaart-")               # to store tool output of running processes
            ...
            os.makedirs(self.runningfolder+'/nxc')                                                  # to store nxc output
            self.usernamesWordlist = Wordlist(self.outputfolder + '/ninjaart-usernames.txt')          # to store found usernames
            self.passwordsWordlist = Wordlist(self.outputfolder + '/ninjaart-passwords.txt')          # to store found passwords
