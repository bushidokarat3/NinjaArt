...
class AppSettings():
    def __init__(self):
        ...
        if not os.path.exists('./ninjaart.conf'):
            print('[+] Creating settings file..')
            self.createDefaultSettings()
        else:
            print('[+] Loading settings file..')
            self.actions = QtCore.QSettings('./ninjaart.conf', QtCore.QSettings.NativeFormat)
    ...
    def createDefaultSettings(self):
        ...
        self.actions.setValue('theme', 'Default')
        self.actions.setValue('font-family', 'Default')
        self.actions.setValue('font-size', '10')
        ...
        self.actions.setValue('lockout-threshold', '3')
        self.actions.setValue('lockout-delay', '1')
        self.actions.setValue('lockout-safe-mode', 'False')
        self.actions.endGroup()
        ...
        self.actions.setValue('nxc-path','/usr/bin/nxc')
#       self.actions.setValue('cutycapt-path','/usr/bin/cutycapt')
        ...
        self.actions.setValue("nxc-smb-scan", ["Scan SMB with nxc", "bash -c \"nxc smb [IP] 2>&1 | tee '[OUTPUT].txt'\""])
        self.actions.setValue("nxc-smb-full", ["Full SMB enum (nxc)", "bash -c \"nxc smb [IP] -u '' -p '' --shares --sessions --disks --users --groups --pass-pol 2>&1 | tee '[OUTPUT].txt'\""])
        self.actions.endGroup()
        ...
        ### NetExec (nxc) - SMB
        self.actions.setValue("nxc-smb-null", ["Check SMB null session (nxc)", "bash -c \"nxc smb [IP] -u '' -p '' 2>&1 | tee '[OUTPUT].txt'\"", "netbios-ssn,microsoft-ds"])
        self.actions.setValue("nxc-smb-guest", ["Check SMB guest access (nxc)", "bash -c \"nxc smb [IP] -u guest -p '' 2>&1 | tee '[OUTPUT].txt'\"", "netbios-ssn,microsoft-ds"])
        self.actions.setValue("nxc-smb-shares", ["Enumerate SMB shares (nxc)", "bash -c \"nxc smb [IP] -u '' -p '' --shares 2>&1 | tee '[OUTPUT].txt'\"", "netbios-ssn,microsoft-ds"])
        self.actions.setValue("nxc-smb-users", ["Enumerate SMB users (nxc)", "bash -c \"nxc smb [IP] -u '' -p '' --users 2>&1 | tee '[OUTPUT].txt'\"", "netbios-ssn,microsoft-ds"])
        self.actions.setValue("nxc-smb-groups", ["Enumerate SMB groups (nxc)", "bash -c \"nxc smb [IP] -u '' -p '' --groups 2>&1 | tee '[OUTPUT].txt'\"", "netbios-ssn,microsoft-ds"])
        self.actions.setValue("nxc-smb-passpol", ["Get password policy (nxc)", "bash -c \"nxc smb [IP] -u '' -p '' --pass-pol 2>&1 | tee '[OUTPUT].txt'\"", "netbios-ssn,microsoft-ds"])
        self.actions.setValue("nxc-smb-signing", ["Check SMB signing (nxc)", "bash -c \"nxc smb [IP] --gen-relay-list '[OUTPUT].txt' 2>&1\"", "netbios-ssn,microsoft-ds"])
        self.actions.setValue("nxc-smb-rid", ["RID brute users (nxc)", "bash -c \"nxc smb [IP] -u '' -p '' --rid-brute 2>&1 | tee '[OUTPUT].txt'\"", "netbios-ssn,microsoft-ds"])
        ### NetExec (nxc) - Other protocols
        self.actions.setValue("nxc-winrm-check", ["Check WinRM access (nxc)", "bash -c \"nxc winrm [IP] -u '' -p '' 2>&1 | tee '[OUTPUT].txt'\"", "wsman,ms-wbt-server"])
        self.actions.setValue("nxc-ldap-enum", ["Enumerate LDAP (nxc)", "bash -c \"nxc ldap [IP] -u '' -p '' 2>&1 | tee '[OUTPUT].txt'\"", "ldap"])
        self.actions.setValue("nxc-mssql-check", ["Check MSSQL access (nxc)", "bash -c \"nxc mssql [IP] -u '' -p '' 2>&1 | tee '[OUTPUT].txt'\"", "ms-sql-s,ms-sql-m"])
        self.actions.setValue("nxc-rdp-check", ["Check RDP access (nxc)", "bash -c \"nxc rdp [IP] 2>&1 | tee '[OUTPUT].txt'\"", "ms-wbt-server"])
        self.actions.setValue("nxc-ssh-check", ["Check SSH access (nxc)", "bash -c \"nxc ssh [IP] -u '' -p '' 2>&1 | tee '[OUTPUT].txt'\"", "ssh"])
        self.actions.setValue("nxc-ftp-check", ["Check FTP access (nxc)", "bash -c \"nxc ftp [IP] -u '' -p '' 2>&1 | tee '[OUTPUT].txt'\"", "ftp"])
        ###
        ...
        self.actions.setValue("nxc-smb-null",["netbios-ssn,microsoft-ds","tcp"])
...
#       self.actions.setValue("nbtscan","netbios-ns")
    ...
    def backupAndSave(self, newSettings):
        ...
        os.rename('./ninjaart.conf', './'+getTimestamp()+'-ninjaart.conf')  
        self.actions = QtCore.QSettings('./ninjaart.conf', QtCore.QSettings.NativeFormat)
        ...
        self.actions.setValue('theme', newSettings.general_theme)
        self.actions.setValue('font-family', newSettings.general_font_family)
        self.actions.setValue('font-size', newSettings.general_font_size)
        self.actions.endGroup()
        ...
        self.actions.setValue('lockout-threshold', newSettings.brute_lockout_threshold)
        self.actions.setValue('lockout-delay', newSettings.brute_lockout_delay)
        self.actions.setValue('lockout-safe-mode', newSettings.brute_lockout_safe_mode)
        self.actions.endGroup()
        ...
        for tool in newSettings.automatedAttacks:
            # automatedAttacks is a list of [name, value1, value2] - save as [value1, value2]
            if len(tool) >= 3:
                self.actions.setValue(tool[0], [tool[1], tool[2]])
            elif len(tool) == 2:
                self.actions.setValue(tool[0], tool[1])
        self.actions.endGroup()
...
class Settings():
    def __init__(self, appSettings=None):
        ...
        self.general_theme = "Default"
        self.general_font_family = "Default"
        self.general_font_size = "10"
        ...
        self.brute_lockout_threshold = "3"
        self.brute_lockout_delay = "1"
        self.brute_lockout_safe_mode = "False"
...
#       self.tools_path_cutycapt = "/usr/bin/cutycapt"
        ...
        if appSettings:
            try:
                ...
                self.general_theme = self.generalSettings.get('theme', 'Default')
                self.general_font_family = self.generalSettings.get('font-family', 'Default')
                self.general_font_size = self.generalSettings.get('font-size', '10')
                ...
                self.brute_lockout_threshold = self.bruteSettings.get('lockout-threshold', '3')
                self.brute_lockout_delay = self.bruteSettings.get('lockout-delay', '1')
                self.brute_lockout_safe_mode = self.bruteSettings.get('lockout-safe-mode', 'False')
