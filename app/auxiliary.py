...
#               print('[+] Taking screenshot of '+url)
    ...
    def save(self, url, ip, port, outputfile):
        ...
        # Try gowitness first (preferred), then fallback to wkhtmltoimage
        if self._try_gowitness(url, out):
            pass
        elif self._try_wkhtmltoimage(url, out):
            pass
        elif self._try_chromium(url, out):
            pass
        else:
            raise Exception("No screenshot tool available. Install gowitness, wkhtmltoimage, or chromium.")
    ...
    def _try_gowitness(self, url, out):
        """Try taking screenshot with gowitness v3"""
        try:
            import glob
            outdir = os.path.dirname(out)
            outfile = os.path.basename(out)

            # gowitness v3 syntax: gowitness scan single -u <url> -s <path>
            command = f"gowitness scan single -u {url} -s {outdir} --write-none --screenshot-format jpeg -T 30"
            p = subprocess.Popen(command, shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
            p.wait(timeout=45)

            # gowitness creates files with its own naming, find and rename
            screenshots = glob.glob(f"{outdir}/*.jpeg") + glob.glob(f"{outdir}/*.jpg") + glob.glob(f"{outdir}/*.png")
            if screenshots:
                newest = max(screenshots, key=os.path.getctime)
                if newest and os.path.exists(newest) and os.stat(newest).st_size > 0:
                    # Rename to expected output filename
                    if newest != out:
                        os.rename(newest, out)
                    return True
        except Exception as e:
            print(f'[-] gowitness failed: {e}')
        return False

    def _try_wkhtmltoimage(self, url, out):
        """Try taking screenshot with wkhtmltoimage"""
        try:
            command = f"wkhtmltoimage --load-error-handling ignore {url} {out}"
            p = subprocess.Popen(command, shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
            p.wait(timeout=30)
            return os.path.exists(out) and os.stat(out).st_size > 0
        except Exception as e:
            print(f'[-] wkhtmltoimage failed: {e}')
        return False

    def _try_chromium(self, url, out):
        """Try taking screenshot with chromium headless"""
        try:
            command = f"chromium --headless --disable-gpu --screenshot={out} --window-size=1280,1024 --no-sandbox {url}"
            p = subprocess.Popen(command, shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
            p.wait(timeout=30)
            return os.path.exists(out) and os.stat(out).st_size > 0
        except Exception as e:
...
def sanitise(string):                                                   # this function makes a string safe for use in sql query. the main point is to prevent ninjaart from breaking, not so much SQLi as such.
    s = string.replace('\'', '\'\'')
    return s
