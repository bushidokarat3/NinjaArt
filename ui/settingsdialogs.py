...
class AddSettingsDialog(QDialog):                                 # dialog shown when the user selects settings menu
    ...
    def updateSettings(self):                                           # updates the local settings object (must be called when applying settings and only after validation succeeded)
        ...
        self.settings.general_theme = str(self.themeComboBox.currentText())
        self.settings.general_font_family = str(self.fontComboBox.currentText())
        self.settings.general_font_size = str(self.fontSizeComboBox.currentText())
    ...
    def populateGeneralTab(self):
        ...
        theme_idx = self.themeComboBox.findText(str(self.settings.general_theme))
        if theme_idx >= 0:
            self.themeComboBox.setCurrentIndex(theme_idx)

        # Populate font family setting
        font_idx = self.fontComboBox.findText(str(self.settings.general_font_family))
        if font_idx >= 0:
            self.fontComboBox.setCurrentIndex(font_idx)

        # Populate font size setting
        size_idx = self.fontSizeComboBox.findText(str(self.settings.general_font_size))
        if size_idx >= 0:
            self.fontSizeComboBox.setCurrentIndex(size_idx)     
...
#           self.terminalCommandText.setStyleSheet("border: 1px solid grey;")
    ...
    def setupGeneralTab(self):
        ...
        self.themeLabel.setFixedWidth(150)
        self.themeComboBox = QComboBox()
        self.themeComboBox.insertItems(0, ['Default', 'Dark', 'Light', 'Hacker', 'Midnight'])
        self.themeComboBox.setFixedWidth(150)
        self.themeComboBox.setStyleSheet("QComboBox { combobox-popup: 0; }")
        self.hlayoutTheme = QHBoxLayout()
        self.hlayoutTheme.addWidget(self.themeLabel)
        self.hlayoutTheme.addWidget(self.themeComboBox)
        self.hlayoutTheme.addStretch()

        # Appearance settings - Font Family
        self.fontLabel = QLabel()
        self.fontLabel.setText('Font Family')
        self.fontLabel.setFixedWidth(150)
        self.fontComboBox = QComboBox()
        self.fontComboBox.insertItems(0, ['Default', 'Monospace', 'Consolas', 'Courier New', 'DejaVu Sans Mono', 'Fira Code', 'Ubuntu Mono', 'Source Code Pro'])
        self.fontComboBox.setFixedWidth(150)
        self.fontComboBox.setStyleSheet("QComboBox { combobox-popup: 0; }")
        self.hlayoutFont = QHBoxLayout()
        self.hlayoutFont.addWidget(self.fontLabel)
        self.hlayoutFont.addWidget(self.fontComboBox)
        self.hlayoutFont.addStretch()

        # Appearance settings - Font Size
        self.fontSizeLabel = QLabel()
        self.fontSizeLabel.setText('Font Size')
        self.fontSizeLabel.setFixedWidth(150)
        self.fontSizeComboBox = QComboBox()
        self.fontSizeComboBox.insertItems(0, ['8', '9', '10', '11', '12', '13', '14', '16', '18', '20'])
        self.fontSizeComboBox.setFixedWidth(150)
        self.fontSizeComboBox.setCurrentIndex(2)  # Default to 10
        self.fontSizeComboBox.setStyleSheet("QComboBox { combobox-popup: 0; }")
        self.hlayoutFontSize = QHBoxLayout()
        self.hlayoutFontSize.addWidget(self.fontSizeLabel)
        self.hlayoutFontSize.addWidget(self.fontSizeComboBox)
        self.hlayoutFontSize.addStretch()
        ...
        self.vlayoutGeneral.addLayout(self.hlayoutTheme)
        self.vlayoutGeneral.addLayout(self.hlayoutFont)
        self.vlayoutGeneral.addLayout(self.hlayoutFontSize)

        self.generalSpacer = QSpacerItem(10,250)
