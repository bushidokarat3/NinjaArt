...
from PyQt5.QtWidgets import QPushButton, QRadioButton, QComboBox, QGroupBox, QButtonGroup, QFileDialog, QSpinBox
...
#       self.labelPath.setText('/')
        ...
        self.exportButton.setMaximumSize(80, 30)
        self.exportButton.setToolTip('Export output to file')
        self.exportButton.clicked.connect(self.exportLogs)

        self.clearButton = QPushButton('Clear')
        self.clearButton.setMaximumSize(80, 30)
        self.clearButton.setToolTip('Clear output display')
        ...
        # Lockout protection controls
        self.checkLockoutSafe = QCheckBox()
        self.checkLockoutSafe.setText('Lockout Safe')
        self.checkLockoutSafe.setToolTip('Rate limit attempts to avoid triggering account lockout policies')
        if self.settings.brute_lockout_safe_mode == 'True':
            self.checkLockoutSafe.toggle()

        self.labelThreshold = QLabel()
        self.labelThreshold.setText('Attempts:')
        self.labelThreshold.setFixedWidth(55)
        self.thresholdSpinBox = QSpinBox()
        self.thresholdSpinBox.setRange(1, 100)
        self.thresholdSpinBox.setValue(int(self.settings.brute_lockout_threshold))
        self.thresholdSpinBox.setFixedWidth(50)
        self.thresholdSpinBox.setToolTip('Number of attempts before waiting')

        self.labelDelay = QLabel()
        self.labelDelay.setText('Wait (min):')
        self.labelDelay.setFixedWidth(60)
        self.delaySpinBox = QSpinBox()
        self.delaySpinBox.setRange(1, 60)
        self.delaySpinBox.setValue(int(self.settings.brute_lockout_delay))
        self.delaySpinBox.setFixedWidth(50)
        self.delaySpinBox.setToolTip('Minutes to wait between attempt batches')

        # Initially hide lockout options if not enabled
        self.labelThreshold.setVisible(self.checkLockoutSafe.isChecked())
        self.thresholdSpinBox.setVisible(self.checkLockoutSafe.isChecked())
        self.labelDelay.setVisible(self.checkLockoutSafe.isChecked())
        self.delaySpinBox.setVisible(self.checkLockoutSafe.isChecked())

        self.checkLockoutSafe.stateChanged.connect(self.toggleLockoutOptions)
        ...
        self.hlayout4.addWidget(self.checkLockoutSafe)
        self.hlayout4.addWidget(self.labelThreshold)
        self.hlayout4.addWidget(self.thresholdSpinBox)
        self.hlayout4.addWidget(self.labelDelay)
        self.hlayout4.addWidget(self.delaySpinBox)
...
#           self.labelPath.hide()
    ...
    def toggleLockoutOptions(self):
        visible = self.checkLockoutSafe.isChecked()
        self.labelThreshold.setVisible(visible)
        self.thresholdSpinBox.setVisible(visible)
        self.labelDelay.setVisible(visible)
        self.delaySpinBox.setVisible(visible)
    ...
    def buildHydraCommand(self, runningfolder, userlistPath, passlistPath):
        ...
        # Lockout protection: rate limit attempts to avoid triggering account lockout policies
        if self.checkLockoutSafe.isChecked():
            threshold = self.thresholdSpinBox.value()
            delay_minutes = self.delaySpinBox.value()
            delay_seconds = delay_minutes * 60
            # Calculate seconds per attempt: total delay / threshold attempts
            # This ensures we don't exceed threshold attempts within the delay window
            seconds_per_attempt = max(1, delay_seconds // threshold)
            self.command += " -c "+str(seconds_per_attempt)  # wait time per login attempt
            self.command += " -t 1"  # use single thread to ensure sequential attempts
            self.command += " -W "+str(seconds_per_attempt)  # wait between connections
        else:
...
#       if self.labelPath.isVisible():                                  # append the additional field's content, if it was visible
    ...
    def exportLogs(self):
        """Export the tool output to a file"""
        content = self.display.toPlainText()
        if not content.strip():
            QMessageBox.information(self, "Export", "No output to export.")
            return

        # Generate default filename
        service = str(self.serviceComboBox.currentText())
        default_name = "hydra-" + self.ip + "-" + service + "-" + getTimestamp() + ".txt"
        ...
        if filename:
            try:
                with open(filename, 'w') as f:
                    f.write(content)
                QMessageBox.information(self, "Export", "Output exported successfully to:\n" + filename)
...
# Widget for NetExec (nxc) / CrackMapExec operations
class NxcWidget(QWidget):
    ...
    def setupLayout(self):
        # Map service names to nxc protocols
        self.protocol = 'smb'
        if self.service in ['netbios-ssn', 'microsoft-ds']:
            self.protocol = 'smb'
        elif self.service in ['ldap', 'ldaps']:
            self.protocol = 'ldap'
        elif self.service in ['ms-sql-s', 'ms-sql-m', 'mssql']:
            self.protocol = 'mssql'
        elif self.service in ['ms-wbt-server', 'rdp']:
            self.protocol = 'rdp'
        elif self.service == 'ssh':
            self.protocol = 'ssh'
        elif self.service == 'ftp':
            self.protocol = 'ftp'
        elif self.service in ['wsman', 'winrm']:
            self.protocol = 'winrm'

        # Row 1: IP, Port, Protocol, Run button
        ...
        self.label3 = QLabel('Protocol')
        self.protocolComboBox = QComboBox()
        self.protocolComboBox.insertItems(0, ['smb', 'winrm', 'ldap', 'mssql', 'rdp', 'ssh', 'ftp', 'vnc', 'wmi'])
        self.protocolComboBox.setStyleSheet("QComboBox { combobox-popup: 0; }")
        # Select the detected protocol
        idx = self.protocolComboBox.findText(self.protocol)
        if idx >= 0:
            self.protocolComboBox.setCurrentIndex(idx)
        ...
        self.validationLabel = QLabel(self)
        self.validationLabel.setText('Invalid input!')
        self.validationLabel.setStyleSheet('QLabel { color: red }')
        self.validationLabel.hide()

        self.hlayout1 = QHBoxLayout()
        self.hlayout1.addWidget(self.label1)
        self.hlayout1.addWidget(self.ipTextinput)
        self.hlayout1.addWidget(self.label2)
        self.hlayout1.addWidget(self.portTextinput)
        self.hlayout1.addWidget(self.label3)
        self.hlayout1.addWidget(self.protocolComboBox)
        self.hlayout1.addWidget(self.runButton)
        self.hlayout1.addWidget(self.exportButton)
        self.hlayout1.addWidget(self.clearButton)
        self.hlayout1.addWidget(self.validationLabel)
        self.hlayout1.addStretch()

        # Row 2: Username options (single or list)
        self.singleUserRadio = QRadioButton()
        self.label4 = QLabel('Username')
        self.label4.setFixedWidth(60)
        self.usernameInput = QLineEdit()
        self.usernameInput.setFixedWidth(125)
        self.usernameInput.setPlaceholderText("or empty for null")

        self.userListRadio = QRadioButton()
        self.labelUserList = QLabel('User List')
        self.labelUserList.setFixedWidth(55)
        self.userListInput = QLineEdit()
        self.userListInput.setFixedWidth(200)
        self.userListInput.setPlaceholderText("/path/to/users.txt")
        self.browseUsersButton = QPushButton('Browse')
        self.browseUsersButton.setMaximumSize(60, 25)
        self.browseUsersButton.clicked.connect(lambda: self.browseFile('user'))

        self.userGroup = QButtonGroup()
        self.userGroup.addButton(self.singleUserRadio)
        self.userGroup.addButton(self.userListRadio)
        self.singleUserRadio.toggle()

        # Connect text edits to toggle radio buttons
        self.usernameInput.textEdited.connect(self.singleUserRadio.toggle)
        self.userListInput.textEdited.connect(self.userListRadio.toggle)

        self.hlayout2 = QHBoxLayout()
        self.hlayout2.addWidget(self.singleUserRadio)
        self.hlayout2.addWidget(self.label4)
        self.hlayout2.addWidget(self.usernameInput)
        self.hlayout2.addWidget(self.userListRadio)
        self.hlayout2.addWidget(self.labelUserList)
        self.hlayout2.addWidget(self.userListInput)
        self.hlayout2.addWidget(self.browseUsersButton)
        self.hlayout2.addStretch()

        # Row 2b: Password options (single or list) and Hash
        self.singlePassRadio = QRadioButton()
        self.label5 = QLabel('Password')
        self.label5.setFixedWidth(60)
        self.passwordInput = QLineEdit()
        self.passwordInput.setFixedWidth(125)
        self.passwordInput.setPlaceholderText("or empty for null")

        self.passListRadio = QRadioButton()
        self.labelPassList = QLabel('Pass List')
        self.labelPassList.setFixedWidth(55)
        self.passListInput = QLineEdit()
        self.passListInput.setFixedWidth(200)
        self.passListInput.setPlaceholderText("/path/to/passwords.txt")
        self.browsePassButton = QPushButton('Browse')
        self.browsePassButton.setMaximumSize(60, 25)
        self.browsePassButton.clicked.connect(lambda: self.browseFile('pass'))

        self.passGroup = QButtonGroup()
        self.passGroup.addButton(self.singlePassRadio)
        self.passGroup.addButton(self.passListRadio)
        self.singlePassRadio.toggle()

        # Connect text edits to toggle radio buttons
        self.passwordInput.textEdited.connect(self.singlePassRadio.toggle)
        self.passListInput.textEdited.connect(self.passListRadio.toggle)

        self.label6 = QLabel('NTLM Hash')
        self.label6.setFixedWidth(70)
        self.hashInput = QLineEdit()
        self.hashInput.setFixedWidth(250)
        self.hashInput.setPlaceholderText("LM:NT or NT only (overrides password)")

        self.hlayout2b = QHBoxLayout()
        self.hlayout2b.addWidget(self.singlePassRadio)
        self.hlayout2b.addWidget(self.label5)
        self.hlayout2b.addWidget(self.passwordInput)
        self.hlayout2b.addWidget(self.passListRadio)
        self.hlayout2b.addWidget(self.labelPassList)
        self.hlayout2b.addWidget(self.passListInput)
        self.hlayout2b.addWidget(self.browsePassButton)
        self.hlayout2b.addWidget(self.label6)
        self.hlayout2b.addWidget(self.hashInput)
        self.hlayout2b.addStretch()

        # Row 3: Domain, Auth options, Lockout controls
        self.label7 = QLabel('Domain')
        self.label7.setFixedWidth(50)
        self.domainInput = QLineEdit()
        self.domainInput.setFixedWidth(125)
        self.domainInput.setPlaceholderText("WORKGROUP")

        self.checkLocalAuth = QCheckBox('Local Auth')
        self.checkLocalAuth.setToolTip('Authenticate to local account instead of domain')

        # Lockout protection controls
        self.checkLockoutSafe = QCheckBox('Lockout Safe')
        self.checkLockoutSafe.setToolTip('Rate limit attempts to avoid triggering account lockout policies')
        if self.settings.brute_lockout_safe_mode == 'True':
            self.checkLockoutSafe.toggle()

        self.labelThreshold = QLabel('Attempts:')
        self.labelThreshold.setFixedWidth(55)
        self.thresholdSpinBox = QSpinBox()
        self.thresholdSpinBox.setRange(1, 100)
        self.thresholdSpinBox.setValue(int(self.settings.brute_lockout_threshold))
        ...
        self.labelDelay = QLabel('Wait (min):')
        self.labelDelay.setFixedWidth(60)
        self.delaySpinBox = QSpinBox()
        self.delaySpinBox.setRange(1, 60)
        self.delaySpinBox.setValue(int(self.settings.brute_lockout_delay))
        ...
        # Continue on success and hide failures options
        self.checkContinueOnSuccess = QCheckBox('Continue on Success')
        self.checkContinueOnSuccess.setToolTip('Continue trying credentials after finding valid ones')

        self.checkHideFailures = QCheckBox('Hide Failures')
        self.checkHideFailures.setToolTip('Only show successful authentication attempts in output')

        # Module selector
        self.label8 = QLabel('Module')
        self.label8.setFixedWidth(45)
        self.moduleComboBox = QComboBox()
        self.moduleComboBox.setFixedWidth(150)
        self.moduleComboBox.setStyleSheet("QComboBox { combobox-popup: 0; }")
        # Common SMB modules
        smb_modules = ['(none)', 'enum_av', 'enum_ca', 'ms17-010', 'zerologon', 'petitpotam',
                       'printnightmare', 'nopac', 'spooler', 'webdav', 'smbghost', 'gpp_password',
                       'gpp_autologin', 'lsassy', 'nanodump', 'spider_plus']
        self.moduleComboBox.insertItems(0, smb_modules)
        self.moduleComboBox.setToolTip('Run a specific nxc module')

        self.hlayout3 = QHBoxLayout()
        self.hlayout3.addWidget(self.label7)
        self.hlayout3.addWidget(self.domainInput)
        self.hlayout3.addWidget(self.checkLocalAuth)
        self.hlayout3.addWidget(self.checkContinueOnSuccess)
        self.hlayout3.addWidget(self.checkHideFailures)
        self.hlayout3.addWidget(self.checkLockoutSafe)
        self.hlayout3.addWidget(self.labelThreshold)
        self.hlayout3.addWidget(self.thresholdSpinBox)
        self.hlayout3.addWidget(self.labelDelay)
        self.hlayout3.addWidget(self.delaySpinBox)
        self.hlayout3.addWidget(self.label8)
        self.hlayout3.addWidget(self.moduleComboBox)
        self.hlayout3.addStretch()

        # Row 4: SMB enumeration options
        self.checkShares = QCheckBox('Shares')
        self.checkShares.setToolTip('Enumerate shares')
        self.checkUsers = QCheckBox('Users')
        self.checkUsers.setToolTip('Enumerate users')
        self.checkGroups = QCheckBox('Groups')
        self.checkGroups.setToolTip('Enumerate groups')
        self.checkSessions = QCheckBox('Sessions')
        self.checkSessions.setToolTip('Enumerate active sessions')
        self.checkDisks = QCheckBox('Disks')
        self.checkDisks.setToolTip('Enumerate disks')
        self.checkPassPol = QCheckBox('Pass Policy')
        self.checkPassPol.setToolTip('Get password policy')
        self.checkRidBrute = QCheckBox('RID Brute')
        self.checkRidBrute.setToolTip('Brute force RIDs to enumerate users')
        self.checkSigning = QCheckBox('Signing')
        self.checkSigning.setToolTip('Check SMB signing status')

        self.hlayout4 = QHBoxLayout()
        self.hlayout4.addWidget(QLabel('Enumerate:'))
        self.hlayout4.addWidget(self.checkShares)
        self.hlayout4.addWidget(self.checkUsers)
        self.hlayout4.addWidget(self.checkGroups)
        self.hlayout4.addWidget(self.checkSessions)
        self.hlayout4.addWidget(self.checkDisks)
        self.hlayout4.addWidget(self.checkPassPol)
        self.hlayout4.addWidget(self.checkRidBrute)
        self.hlayout4.addWidget(self.checkSigning)
        self.hlayout4.addStretch()

        # Row 5: Additional options
        self.checkSam = QCheckBox('Dump SAM')
        self.checkSam.setToolTip('Dump SAM hashes (requires admin)')
        self.checkLsa = QCheckBox('Dump LSA')
        self.checkLsa.setToolTip('Dump LSA secrets (requires admin)')
        self.checkNtds = QCheckBox('Dump NTDS')
        self.checkNtds.setToolTip('Dump NTDS.dit (requires domain admin)')

        self.hlayout5 = QHBoxLayout()
        self.hlayout5.addWidget(QLabel('Dump:'))
        self.hlayout5.addWidget(self.checkSam)
        self.hlayout5.addWidget(self.checkLsa)
        self.hlayout5.addWidget(self.checkNtds)
        self.hlayout5.addStretch()

        # Row 6: Command execution options
        self.labelExecCmd = QLabel('Exec CMD (-x):')
        self.labelExecCmd.setFixedWidth(85)
        self.execCmdInput = QLineEdit()
        self.execCmdInput.setFixedWidth(300)
        self.execCmdInput.setPlaceholderText("e.g., whoami, ipconfig, net user")
        self.execCmdInput.setToolTip('Execute a command via cmd.exe (requires admin)')

        self.labelExecPS = QLabel('Exec PS (-X):')
        self.labelExecPS.setFixedWidth(80)
        self.execPSInput = QLineEdit()
        self.execPSInput.setFixedWidth(300)
        self.execPSInput.setPlaceholderText("e.g., Get-Process, Get-Service")
        self.execPSInput.setToolTip('Execute a PowerShell command (requires admin)')

        self.hlayout6 = QHBoxLayout()
        self.hlayout6.addWidget(self.labelExecCmd)
        self.hlayout6.addWidget(self.execCmdInput)
        self.hlayout6.addWidget(self.labelExecPS)
        self.hlayout6.addWidget(self.execPSInput)
        ...
        self.vlayout.addLayout(self.hlayout2)
        self.vlayout.addLayout(self.hlayout2b)
        self.vlayout.addLayout(self.hlayout3)
        self.vlayout.addLayout(self.hlayout4)
    ...
    def buildNxcCommand(self, runningfolder):
        self.ip = self.ipTextinput.text()
        self.port = self.portTextinput.text()
        protocol = str(self.protocolComboBox.currentText())

        self.outputfile = runningfolder + "/nxc/" + getTimestamp() + "-" + self.ip + "-" + self.port + "-" + protocol + ".txt"

        # Base command
        self.command = "nxc " + protocol + " " + self.ip

        # Add port if not default
        if self.port:
            self.command += " --port " + self.port

        # Authentication - username (single or list)
        if self.userListRadio.isChecked() and self.userListInput.text().strip():
            userlist = self.userListInput.text().strip()
            self.command += " -u '" + userlist + "'"
        else:
            username = self.usernameInput.text().strip()
            if username:
                self.command += " -u '" + username + "'"
            else:
                self.command += " -u ''"

        # Authentication - password/hash (single or list)
        ntlm_hash = self.hashInput.text().strip()
        if ntlm_hash:
            self.command += " -H '" + ntlm_hash + "'"
        elif self.passListRadio.isChecked() and self.passListInput.text().strip():
            passlist = self.passListInput.text().strip()
            self.command += " -p '" + passlist + "'"
        else:
            password = self.passwordInput.text().strip()
            if password:
                self.command += " -p '" + password + "'"
            else:
                self.command += " -p ''"

        domain = self.domainInput.text().strip()
        if domain:
            self.command += " -d '" + domain + "'"

        if self.checkLocalAuth.isChecked():
            self.command += " --local-auth"

        # Lockout protection: add jitter/delay between attempts
        if self.checkLockoutSafe.isChecked():
            threshold = self.thresholdSpinBox.value()
            delay_minutes = self.delaySpinBox.value()
            delay_seconds = delay_minutes * 60
            # Calculate jitter: nxc uses --jitter for random delay between attempts
            # We'll use the delay as max jitter value
            jitter_per_attempt = max(1, delay_seconds // threshold)
            self.command += " --jitter " + str(jitter_per_attempt)

        # Continue on success - keep trying other credentials after finding valid ones
        if self.checkContinueOnSuccess.isChecked():
            self.command += " --continue-on-success"

        # Enumeration options (SMB specific)
        if protocol == 'smb':
            if self.checkShares.isChecked():
                self.command += " --shares"
            if self.checkUsers.isChecked():
                self.command += " --users"
            if self.checkGroups.isChecked():
                self.command += " --groups"
            if self.checkSessions.isChecked():
                self.command += " --sessions"
            if self.checkDisks.isChecked():
                self.command += " --disks"
            if self.checkPassPol.isChecked():
                self.command += " --pass-pol"
            if self.checkRidBrute.isChecked():
                self.command += " --rid-brute"
            if self.checkSigning.isChecked():
                self.command += " --gen-relay-list '" + self.outputfile + ".relay'"

            # Dump options (require admin)
            if self.checkSam.isChecked():
                self.command += " --sam"
            if self.checkLsa.isChecked():
                self.command += " --lsa"
            if self.checkNtds.isChecked():
                self.command += " --ntds"

        # Module support
        module = str(self.moduleComboBox.currentText())
        if module and module != '(none)':
            self.command += " -M " + module

        # Command execution options
        exec_cmd = self.execCmdInput.text().strip()
        exec_ps = self.execPSInput.text().strip()
        if exec_cmd:
            # Escape single quotes in the command
            exec_cmd_escaped = exec_cmd.replace("'", "'\\''")
            self.command += " -x '" + exec_cmd_escaped + "'"
        if exec_ps:
            # Escape single quotes in the PowerShell command
            exec_ps_escaped = exec_ps.replace("'", "'\\''")
            self.command += " -X '" + exec_ps_escaped + "'"

        # Wrap in bash for proper output handling
        # If hide failures is checked, filter out lines containing [-] (failed attempts)
        if self.checkHideFailures.isChecked():
            # Filter: show lines with [+] (success), [*] (info), or lines without status indicators
            self.command = "bash -c \"" + self.command + " 2>&1 | grep -v '\\[-\\]' | tee '" + self.outputfile + "'\""
        else:
            self.command = "bash -c \"" + self.command + " 2>&1 | tee '" + self.outputfile + "'\""
    ...
    def exportLogs(self):
        ...
        # Generate default filename
        protocol = str(self.protocolComboBox.currentText())
        default_name = "nxc-" + self.ip + "-" + protocol + "-" + getTimestamp() + ".txt"
    ...
    def browseFile(self, filetype):
        ...
        if filetype == 'user':
            title = 'Choose username list'
            default_path = self.settings.brute_username_wordlist_path
        else:
            title = 'Choose password list'
            default_path = self.settings.brute_password_wordlist_path

        filename, _ = QFileDialog.getOpenFileName(self, title, default_path)
        if filename:
            if filetype == 'user':
                self.userListInput.setText(filename)
                self.userListRadio.toggle()
            else:
                self.passListInput.setText(filename)
                self.passListRadio.toggle()

    def toggleLockoutOptions(self):
        """Show/hide lockout options based on checkbox state"""
        visible = self.checkLockoutSafe.isChecked()
        self.labelThreshold.setVisible(visible)
        self.thresholdSpinBox.setVisible(visible)
        self.labelDelay.setVisible(visible)
