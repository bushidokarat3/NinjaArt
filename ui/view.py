...
#from ui.gui import *
from ui.dialogs import HostInformationWidget, FiltersDialog, ProgressWidget, AddHostsDialog, ImagePlayer, ImageViewer, BruteWidget, NxcWidget
from ui.settingsdialogs import AddSettingsDialog
...
class View(QObject):
    ...
    def __init__(self, ui, ui_mainwindow):
        ...
        self.startOnce()                                                # initialisations that happen only once, when NinjaArt is launched
    ...
    def startOnce(self):
        ...
        if not os.path.exists(self.helpurl):
            self.helpurl = '/usr/share/doc/ninjaart/help.html'
    ...
    def start(self, title='*untitled'):
        ...
        self.nxcTabCount = 1                                            # to keep track of the numbering of the nxc tabs
        ...
        self.resetNxcTabs()                                             # clear nxc tabs (if any) and create default nxc tab
    ...
    def startConnections(self):                                         # signal initialisations (signals/slots, actions, etc)
        ...
        self.ui.NxcTabWidget.tabCloseRequested.connect(self.closeNxcTab)
    ...
    def openExistingProject(self):      
        if self.dealWithCurrentProject():
            filename = QFileDialog.getOpenFileName(self.ui.centralwidget, 'Open project', self.controller.getCWD(), filter='NinjaArt project (*.sprt)')[0]
    ...
    def saveProjectAs(self):
        ...
        filename = QFileDialog.getSaveFileName(self.ui.centralwidget, 'Save project as', self.controller.getCWD(), filter='NinjaArt project (*.sprt)', options=QFileDialog.DontConfirmOverwrite)[0]
        ...
        while not filename =='':
            ...
            filename = QFileDialog.getSaveFileName(self.ui.centralwidget, 'Save project as', '.', filter='NinjaArt project (*.sprt)', options=QFileDialog.DontConfirmOverwrite)[0]
    ...
    def showSettingsWidget(self):
        self.settingsWidget.setSettings(self.controller.getSettings())
    ...
    def applyAppearance(self, settings):
        """Apply theme and font settings immediately"""
        ...
        themes = {
            ...
            'Dark': '''
                ...
                QSpinBox { background-color: #1e1e1e; color: #ffffff; border: 1px solid #555; }
            ...
            'Hacker': '''
                ...
                QComboBox QAbstractItemView { background-color: #0a0a0a; color: #00ff00; }
                ...
                QSpinBox { background-color: #000000; color: #00ff00; border: 1px solid #00ff00; }
                QHeaderView::section { background-color: #0a0a0a; color: #00ff00; border: 1px solid #00ff00; }
                QScrollBar { background-color: #0a0a0a; }
            ''',
            'Midnight': '''
                ...
                QComboBox QAbstractItemView { background-color: #16213e; color: #eaeaea; }
                ...
                QSpinBox { background-color: #16213e; color: #eaeaea; border: 1px solid #0f3460; }
                QHeaderView::section { background-color: #0f3460; color: #eaeaea; }
                QScrollBar { background-color: #1a1a2e; }
            '''
        }
        ...
        try:
            with open('./ui/ninjaart.qss', 'r') as f:
                base_qss = f.read()
        except:
            try:
                with open('./ui/ninjaart.qss', 'r') as f:
        ...
        # Apply theme
        if theme in themes:
            full_stylesheet = base_qss + themes[theme]
        else:
            full_stylesheet = base_qss

        self.ui_mainwindow.setStyleSheet(full_stylesheet)
        ...
        # Force update
        self.ui_mainwindow.update()
        print(f'[+] Applied theme: {theme}, font: {font_family}, size: {font_size}')
    ...
    def createNewNxcTab(self, ip, port, service):
        self.ui.statusbar.showMessage('Sending to NXC: '+ip+':'+port+' ('+service+')', msecs=1000)
        nWidget = NxcWidget(ip, port, service, self.controller.getSettings())
        nWidget.runButton.clicked.connect(lambda: self.callNxc(nWidget))
        self.ui.NxcTabWidget.addTab(nWidget, str(self.nxcTabCount))
        self.nxcTabCount += 1
        self.ui.NxcTabWidget.setCurrentIndex(self.ui.NxcTabWidget.count()-1)

    def closeNxcTab(self, index):
        currentTabIndex = self.ui.NxcTabWidget.currentIndex()
        self.ui.NxcTabWidget.setCurrentIndex(index)

        if not self.ui.NxcTabWidget.currentWidget().pid == -1:
            if self.ProcessesTableModel.getProcessStatusForPid(self.ui.NxcTabWidget.currentWidget().pid)=="Running":
                message = "This process is still running. Are you sure you want to kill it?"
                reply = QMessageBox.question(self.ui.centralwidget, 'Confirm', message, QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
                if reply == QMessageBox.Yes:
                    self.killNxcProcess(self.ui.NxcTabWidget.currentWidget())
                else:
                    return

        dbIdString = self.ui.NxcTabWidget.currentWidget().display.property('dbId')
        if dbIdString and not dbIdString == '':
            self.controller.storeCloseTabStatusInDB(int(dbIdString))

        self.ui.NxcTabWidget.removeTab(index)

        if currentTabIndex >= self.ui.NxcTabWidget.currentIndex():
            self.ui.NxcTabWidget.setCurrentIndex(currentTabIndex - 1)
        else:
            self.ui.NxcTabWidget.setCurrentIndex(currentTabIndex)

        if self.ui.NxcTabWidget.count() == 0:
            self.createNewNxcTab('127.0.0.1', '445', 'microsoft-ds')

    def resetNxcTabs(self):
        count = self.ui.NxcTabWidget.count()
        for i in range(0, count):
            self.ui.NxcTabWidget.removeTab(count - i - 1)
        self.createNewNxcTab('127.0.0.1', '445', 'microsoft-ds')

    def callNxc(self, nWidget):
        if validateNmapInput(nWidget.ipTextinput.text()) and validateNmapInput(nWidget.portTextinput.text()):
            if not self.controller.isHostInDB(nWidget.ipTextinput.text()):
                message = "This host is not in scope. Add it to scope and continue?"
                reply = QMessageBox.question(self.ui.centralwidget, 'Confirm', message, QMessageBox.Yes | QMessageBox.No, QMessageBox.Yes)
                if reply == QMessageBox.No:
                    return
                else:
                    self.controller.addHosts(str(nWidget.ipTextinput.text()), False, False)

            nWidget.validationLabel.hide()
            nWidget.toggleRunButton()
            nWidget.resetDisplay()

            nxcCommand = nWidget.buildNxcCommand(self.controller.getRunningFolder())
            nWidget.setObjectName(str("nxc"+" ("+nWidget.getPort()+"/tcp)"))

            hosttabs = []
            if str(nWidget.ip) in self.hostTabs:
                hosttabs = self.hostTabs[str(nWidget.ip)]

            hosttabs.append(nWidget)
            self.hostTabs.update({str(nWidget.ip):hosttabs})

            self.controller.runCommand("nxc", nWidget.objectName(), nWidget.ip, nWidget.port, "tcp", nxcCommand, getTimestamp(True), nWidget.outputfile, nWidget.display)
        else:
            nWidget.validationLabel.show()

    def killNxcProcess(self, nWidget):
        dbId = str(nWidget.display.property('dbId'))
        status = self.controller.getProcessStatusForDBId(dbId)
        if status == "Running":
            self.controller.killProcess(self.controller.getPidForProcess(dbId), dbId)
        elif status == "Waiting":
            self.controller.cancelProcess(dbId)
        self.nxcProcessFinished(nWidget)

    def nxcProcessFinished(self, nWidget):
        nWidget.toggleRunButton()
        nWidget.pid = -1

        self.createNewTabForHost(str(nWidget.ip), str(nWidget.objectName()), restoring=True, content=str(nWidget.display.toPlainText())).setProperty('dbId', QVariant(str(nWidget.display.property('dbId'))))

        hosttabs = []
        if str(nWidget.ip) in self.hostTabs:
            hosttabs = self.hostTabs[str(nWidget.ip)]

        if hosttabs.count(nWidget) > 1:
            hosttabs.remove(nWidget)

        self.hostTabs.update({str(nWidget.ip):hosttabs})

        nWidget.runButton.clicked.disconnect()
        nWidget.runButton.clicked.connect(lambda: self.callNxc(nWidget))

    def findFinishedNxcTab(self, pid):
        for i in range(0, self.ui.NxcTabWidget.count()):
            if str(self.ui.NxcTabWidget.widget(i).pid) == pid:
                self.nxcProcessFinished(self.ui.NxcTabWidget.widget(i))
                return
